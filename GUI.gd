...
func dragStart(node: MarginContainer):
	...
	self.ghostTab.position = self.mouse_position - Vector2(self.ghostTab.size.x-70,0)
...
func dragEnd():
	...
	get_child(0).get_child(1).updateDims() # Goes to the TabController.gd script and updates the dims for any necessary changes that may have occured
